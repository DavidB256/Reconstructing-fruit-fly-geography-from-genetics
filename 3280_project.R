...
PC_EU <- cbind(PC_EU, pro$Yrot) %>%
  ...
  mutate(sample_lat_text_adj = case_when(
           country == "Spain" ~ -0.5,
           country == "Germany" ~ -0.7,
           TRUE ~ 0),
         sample_long_text_adj = case_when(
           country == "Germany" ~ -0.5,
           TRUE ~ 0))

map_eu <- map_data("world") %>%
  mutate(sampled = region %in% 
           c("Portugal", "Spain", "France", "Italy", "Switzerland", "German", "Austria", "Netherlands"),
         british_isles = region %in% c("UK", "Ireland", "Algeria", "Tunisia", "Morocco", "Isle of Man"))

# Plot Procrustes analysis results with map in background
ggplot(PC_EU) + 
  geom_polygon(map_eu,
               mapping=aes(x=long, y=lat, group=group, 
                           fill=british_isles, color=british_isles)) +
  scale_fill_manual(values=c("#cccccc", "#acbfe3")) +
  scale_color_manual(values=c("black", "#acbfe3")) +
  geom_point(aes(x=sample_long, y=sample_lat), size=3) +
  geom_text(aes(x=sample_long + sample_long_text_adj, 
                y=sample_lat + sample_lat_text_adj,
                label=country),
            size=5) +
  geom_point(aes(x=proc_long, y=proc_lat), size=3, color="#c40a20") +
  geom_text(aes(x=proc_long, y=proc_lat, label=country),
            color="#c40a20", size=5) +
  geom_curve(aes(x=sample_long, y=sample_lat,
             ...
             curvature=0.2,
             size=1) +
  geom_text(aes(label="Geography of samples", x=-8, y=54), 
            size=6, hjust=0) +
  geom_text(aes(label="Geography reconstructed\nfrom genetics", x=-8, y=52.5), 
            color="#c40a20", size=6, hjust=0) +
  theme_classic() +
  theme(panel.background = element_rect(fill="#acbfe3"),
